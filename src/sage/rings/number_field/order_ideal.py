...
class NumberFieldOrderIdeal_generic(Ideal_generic):
    ...
    """
    def __init__(self, O, gens, *, coerce=True, **_):
        r"""
        ...
        We can lift an ideal $\mathcal{I}$ to this order by taking intersection
        $\mathcal{I} \cap O$.

            sage: K.<a> = QuadraticField(-5)
            sage: O = K.order_of_conductor(37)
            sage: I = K.maximal_ideal([2, a + 1]); I
            Ideal (a + 1, 2*a) of Maximal Order in Number Field in a with defining polynomial x^2 + 5 with a = 2.236067977499790?*I
            sage: I.go_down(O)
            Ideal (37*a + 1, 74*a) of Order in Number Field in a with defining polynomial x^2 + 5 with a = 2.236067977499790?*I
            sage: _ == O.ideal(2, 37 * a + 1)
            True

            sage: x = QQ['x'].0
            sage: K.<a> = NumberField(x^4 + 23)
            sage: O = K.order([1, 3 * a, 7 * a^2])
            sage: I = K.maximal_ideal(19)
            sage: I.go_down(O)
            Ideal (19, 57*a, 19*a^2, 57*a^3) of Order in Number Field in a with defining polynomial x^4 + 23
        """
        ...
        if isinstance(gens, Module_free_ambient):
            self._module = gens
            from_V = O.number_field().absolute_vector_space()[1]
            basis = [O(from_V(v)) for v in gens.basis()]
        ...
        if len(gens) == 1 and isinstance(gens[0], pari_gen):
            ...
            if gens.type() == "t_MAT":
                ...
                gens = [O(x, check=False) for x in O.pari_zk() * gens]
            elif gens.type() == "t_VEC":
                # Assume prime ideal form
                self._pari_prime = gens
                gens = [ZZ(gens.pr_get_p()), O(gens.pr_get_gen())]
            else:
                # Assume one element of the field
                gens = [O(gens, check=False)]

        self._module, basis = self._gens_to_module(O, gens, coerce=coerce, basis=True)
        super().__init__(O, basis, coerce=False)

    def _gens_to_module(self, O, gens, coerce=True, basis=False):
        """
        Return a module corresponding to the space generated by the generators.
        """
        ...
        module = O.free_module().submodule(gens)

        if basis:
            basis = [O(from_V(v)) for v in module.basis()]
            return module, basis
    ...
    def _mul_(self, other):
        ...
        EXAMPLES::
            ...
            sage: A = K.maximal_ideal([5, 2 + I])
            sage: B = K.maximal_ideal([13, 5 + 12*I])
            sage: A*B
            Ideal (18*I + 1, 65*I) of Gaussian Integers in Number Field in I with defining polynomial x^2 + 1 with I = 1*I
            sage: (K.maximal_ideal(3 + I) * K.maximal_ideal(7 + I)).gens_reduced()
            (-10*I - 20,)
            sage: O = K.order_of_conductor(13)
            sage: A.go_down(O) * B.go_down(O)
            Ideal (39*I + 13, 65*I) of Order in Number Field in I with defining polynomial x^2 + 1 with I = 1*I
            sage: _ == (A * B).go_down(O)
            True
        ...
        Make sure that :trac:`13958` is fixed::
            ...
            sage: I = QuadraticField(-5).maximal_ideal(2).factor()[0][0]; I
        ...
        #     return self.ring().ideal(self.gen(0) * other.gen(0), future=True)
        #
        # O = self.ring()
        # return O.ideal(O.pari_nf().idealmul(self, other), future=True)

        # TODO (grhkm): I think this is non-optimal (algorithmically)?
        return self.ring().ideal([g * h for g in self.gens() for h in other.gens()], future=True)
    ...
    def __pari__(self):
        ...
        EXAMPLES::
            ...
            sage: I = K.maximal_ideal(K.class_group().0.ideal()); I
    ...
    def go_up(self, order=None):
        """
        a -> O_K * k. Note that go_down and go_up are *not* inverses when self is not coprime to the conductor.
        ...
        EXAMPLES::
            ...
            sage: O = K.order([13 * a])
            sage: I = K.maximal_ideal([13, a - 8])
            sage: I.go_down(O).go_up() == I
            False
        """
        # TODO (grhkm): Naming, Docs
        if order is None:
            order = self.number_field().maximal_order()
        elif not self.ring().is_suborder(order):
            # TODO (grhkm): What is the correct error type?
            raise NotImplementedError("ideal does not lie in a suborder")
        # TODO (grhkm): We should be able to do O_K * k, but that returns a
        # fractional ideal for maximal order :P
        return order.ideal(self, future=True)

    def go_down(self, order):
        """
        a -> a ∩ order. Note that go_down and go_up are *not* inverses when self is not coprime to the conductor.
        ...
        EXAMPLES::
            ...
            False
        """
        # TODO (grhkm): Rename this...
        return order.ideal(self.free_module() & order.free_module(), future=True)

    def factor(self):
        """
        If this ideal is coprime to the conductor of the order, return
        the factorization of this ideal in terms of prime ideals.

        The reason for the coprime condition is that ideals of non-maximal
        orders do not have unique factorisation.
        ...
        EXAMPLES::
            ...
            sage: F = I.factor(); F
            doctest:warning...DeprecationWarning...
            (Ideal (1/4*a^3 + 1/4*a^2 + 1/4*a + 1/4, 1/2*a^3 + 1/2*a, a^2, a^3) of Maximal Order in Number Field in a with defining polynomial x^4 + 23) * (Ideal (1/4*a^3 + 65/4*a^2 + 1/4*a + 1/4, 29/2*a^3 + 9*a^2 + 1/2*a, 19*a^2, 19*a^3) of Maximal Order in Number Field in a with defining polynomial x^4 + 23) * (Ideal (57/4*a^3 + 29/4*a^2 + 1/4*a + 1/4, 29/2*a^3 + 10*a^2 + 1/2*a, 19*a^2, 19*a^3) of Maximal Order in Number Field in a with defining polynomial x^4 + 23)
            sage: F.prod() == I
            True

        The method does not work for non-invertible ideals i.e. those not
        coprime to the conductor.
            ...
            sage: I = K.maximal_ideal([13, a - 8])
            sage: I.factor()
            (Ideal (1, a) of Gaussian Integers in Number Field in a with defining polynomial x^2 + 1 with a = 1*I) * (Ideal (8*a + 1, 13*a) of Gaussian Integers in Number Field in a with defining polynomial x^2 + 1 with a = 1*I)
        """
        try:
            ...
            for p, e in zip(F[0], F[1]):
                A.append((K.maximal_ideal(p).go_down(order), ZZ(e)))
            # By default, the unit would be order.ideal(1), which would be the
            # fractional ideal when order is O_K, hence I need to supply
            # `unit=`. However, this means that the `_repr_` method will include
            # it in the string, even though it's just 1.
            # TODO (grhkm): Remove `unit=` argument.
            self.__factorization = Factorization(A, unit=order.ideal(1, future=True))
            return self.__factorization
...
class NumberFieldOrderIdeal_quadratic(NumberFieldOrderIdeal_generic):
    ...
    def gens_reduced(self):
        ...
        assert self.ring().ideal(gen, warn=False) == self
        return (gen,)
